...
VsyncSource::VsyncSource()
  : mObserversMutex("VsyncSource Observers Lock")
{
  // generate a UUID
  mID = gfxUtils::GenerateUUID();
}
...
VsyncSource::AddVsyncObserver(VsyncObserver *aObserver)
{
  ...
  { // scope lock
    MutexAutoLock lock(mObserversMutex);
...
{
  AutoTArray<RefPtr<VsyncObserver>,8> observers;
...
VsyncSource::Shutdown()
{
  ...
  VSYNC_LOG("[%s]: %p VsyncSource %s shutdown\n", XRE_GetProcessTypeString(), this, nsIDToCString(mID).get());
}
...
VsyncManager::VsyncManager()
  : mSourcesMutex("VsyncManager Sources mutation monitor")
{
}
...
{
  mSourcesMutex.AssertCurrentThreadOwns();
...
VsyncManager::RegisterSource(VsyncSource* aSource)
{
  ...
  MutexAutoLock lock(mSourcesMutex);
...
{
  MutexAutoLock lock(mSourcesMutex);
  aSources.AppendElements(mSources);
}
...
VsyncManager::Shutdown()
{
  ...
  mSources.Clear();
  VSYNC_LOG("[%s]: %p VsyncManager shutdown finished\n", XRE_GetProcessTypeString(), this);
}
...
void
SoftwareVsyncSource::Tick(mozilla::TimeStamp aVsyncTimestamp)
{
  MOZ_ASSERT(IsInSoftwareVsyncThread());
...
{
  MOZ_ASSERT(IsInSoftwareVsyncThread());
  mozilla::TimeStamp nextVsync = aVsyncTimestamp + mVsyncInterval;
